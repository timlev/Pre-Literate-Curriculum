...
def produce_blob(filename,wordlist, soup):
    ...
    for row in columns:
        for col in row:
            shuffled_answer_div.append(create_answer(col,index))
            shuffled_answer_div.append(create_audio(col,index))
            index += 1
    random.shuffle(shuffled_answer_div)
    for div in shuffled_answer_div:
        answers_div.append(div)
...
#Get words from Learning-English-HTML Newcomer Lessons
for root, dirs, files in os.walk("../../Learning-English-HTML/Units/Newcomer"):
    if "pics" in root:
        lesson = root.replace("/pics", "").replace("../../Learning-English-HTML/Units/Newcomer/", "")
        items = [os.path.splitext(x)[0] for x in files]
        image_files = os.path.join(root)
        print(image_files)
        print(lesson)
        print(items)
        base = "../"
